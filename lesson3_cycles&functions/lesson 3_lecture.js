...
--- while...do - це – цикл із передумовою, тобто "спочатку думаємо - потім робимо". 
Виконується, поки умова істинна. 
Умову перевіряють  перед виконанням тіла. 
...
/*---- do...while - цикл із постумовою, "спочатку робимо, потім думаємо". 
...
Сигнатура функції — це характеристика, яка включає назву функції та кількість і типи її параметрів.

Сигнатура функції зазвичай використовується для визначення того, як викликати функцію і які аргументи їй можна передати. 
Наприклад:
...
У цьому випадку, сигнатура функції sum включає:
- Назву функції: sum
- Кількість параметрів: 2 (a і b)   */
...
/* --- Function Expression (функціональній вираз) --- 

Він дозволяє створювати функцію всередині будь-якого виразу.
Зауважте, після ключового слова function немає назви функції. 
Для функціональних виразів це допустимо. */
...
let q = 5;
function squareAreaCalculation() {
    let result = q * q; // Але все одно погано що ми маємо залежність через використання зовнішньої змінної:
    return result;
    // return result = c * c; // Інший спосіб
}
...
let w = 5;
function squareAreaCalculation() {
    let result = w * w; // Але все одно погано що ми маємо залежність через використання зовнішньої змінної:
    return result;
...
let result = squareAreaCalculation(8);
console.log(result);
...
hello('Zeus', 'Appolon', 32);
hello('Yana', 'Dorofeeva', 82);
...
function myDisplayer(sentence){
    console.log("Result is " + sentence)
}
...
function mySum(sentence){
    console.log("Result Sum is " + sentence*2)
}

function myDisplayerBrowser(sentence){
    console.log("Result in Browser is " + sentence)
}
...
myCalculator(6, 7, myDisplayerBrowser);
myCalculator(6, 7, mySum);
...
myCalculator(5, 4); // Викликаємо функцію де відбувається обчислення

// Ми хочемо поділити логіку виводу та математики
